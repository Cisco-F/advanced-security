...
主机侧镜像服务器负责扫描镜像目录并维护“当前启用镜像”。`/images` 返回可用镜像列表，`/images/select` 根据序号或文件名切换当前镜像；STM32 仍然只访问固定的 `/image` 数据路径，因此切换镜像不需要改固件。
...
强制重启使用同一组控制脚：先拉低 PB3 触发强制关断，再拉低 PB4 触发上电。主机侧 shell 会在切换远程启动镜像后发送 `ForceRestart`，让树莓派重新枚举 STM32 暴露的 USB MSC 启动盘。
